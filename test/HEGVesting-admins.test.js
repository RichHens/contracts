 ...
 * ------------------------------------------------------------------------------
 */
describe('HEG Vesting: Admin access tests', function () {
    ...
    beforeEach(async function () {
        ...
        token = await HEGToken.deploy(0, [], [admin1.address, admin2.address], 1);
    ...
    describe('Request tests', function () {
        ...
        it("not enough requests (requests == MIN_REQUEST_REQUIRED - 1)", async function() {
            ...
            await banAdminFailed(admin1, banAccount, "HEGVesting: Not enough requests.");
            expect(await vesting.isAdmin(banAccount.address))
                .to.be.eq(true);
        });
        ...
        it("two admin requests for the same admin", async function() {
            ...
            await requestAdminBanFailed(admin1, banAccount, "HEGVesting: The request already exists.");
        });
        ...
        it("two bans for the same admin", async function() {
            ...
            expect(await vesting.isAdmin(banAccount.address))
                .to.be.eq(false);
            await banAdminFailed(admin1, banAccount, "HEGVesting: The account is not an admin.");
        });

        // ----------------------------------------------------------------------------
        it("ban not an admin", async function() {
            await requestAdminBanFailed(admin1, notAdmin, "HEGVesting: The account is not an admin.");
        });

        // ----------------------------------------------------------------------------
        it("ban yourself", async function() {
            await requestAdminBanFailed(admin1, admin1, "HEGVesting: It is forbidden to ban yourself.");
        });
    });
    ...
    describe('onlyAdmin tests', function () {
        ...
        it("requestAdminBan", async function() {
            await requestAdminBanFailed(notAdmin, admin1, "HEGVesting: You are not an admin.");
        });

        // ----------------------------------------------------------------------------
        it("revokeAdminBanRequest", async function() {
            await revokeAdminBanRequestFailed(notAdmin, admin1, "HEGVesting: You are not an admin.");
        });

        // ----------------------------------------------------------------------------
        it("banAdmin", async function() {
            await banAdminFailed(notAdmin, admin1, "HEGVesting: You are not an admin.");
        });
    });
