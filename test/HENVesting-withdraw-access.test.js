...
describe('HEN Vesting: Creation vesting access tests', function () {
  ...
  describe('Request tests', function () {
    ...
    it("not enough approvals (approvals == MIN_REQUESTS_REQUIRED - 1)", async function() {
      ...
      await withdrawFailed(admin1, "HENVesting: Not enough approves.");
    });
    ...
    it("try to withdraw a request twice", async function() {
      ...
      await withdrawSuccess(admin1);
      await withdrawFailed(admin1, "HENVesting: Request is already executed.");
    });
  });
  ...
  describe('Approvals tests', function () {
    ...
    it("try to approve twice one request", async function() {
      await requestWithdrawalSuccess(admin1);
      await approveRequestWithdrawalFailed(admin1, "HENVesting: Request is already approved.");
      await approveRequestWithdrawalSuccess(admin2);
      await approveRequestWithdrawalFailed(admin2, "HENVesting: Request is already approved.");
    });

    // ----------------------------------------------------------------------------
    it("try to approve a non-existent request", async function() {
      await requestWithdrawalSuccess(admin1, 0);
      await approveRequestWithdrawalFailed(admin1, "HENVesting: Request does not exist.", 1);
    });
  ...
  describe('Revocation tests', function () {
    ...
    it("try to revoke a non-existing request", async function() {
      await requestWithdrawalSuccess(admin1, 0);
      await revokeWithdrawalRequestFailed(admin1, "HENVesting: Request does not exist.", 1);
    });
    ...
    it("try to revoke already executed request", async function() {
      ...
      await withdrawSuccess(admin1);
      await revokeWithdrawalRequestFailed(admin1, "HENVesting: Request is already executed.");
    });
  });
  ...
  describe('onlyAdmin tests', function () {
    ...
    it("withdraw", async function() {
      await withdrawFailed(notAdmin, "HENVesting: You are not an admin.");
    });

    // ----------------------------------------------------------------------------
    it("requestWithdrawal", async function() {
      await requestWithdrawalFailed(notAdmin, "HENVesting: You are not an admin.");
    });

    // ----------------------------------------------------------------------------
    it("approveRequestWithdrawal", async function() {
      await approveRequestWithdrawalFailed(notAdmin, "HENVesting: You are not an admin.");
    });

    // ----------------------------------------------------------------------------
    it("revokeWithdrawalRequest", async function() {
      await revokeWithdrawalRequestFailed(notAdmin, "HENVesting: You are not an admin.");
    });
  });
