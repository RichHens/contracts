 ...
 * ------------------------------------------------------------------------------
 */
describe('HEG Vesting: Schedule tests', function () {

    beforeEach(async function () {
        [acc1, acc2, acc3] = await ethers.getSigners();
        const HEGToken = await ethers.getContractFactory("HEGToken", acc1);
        token = await HEGToken.deploy(0, [[0, 1000000]], [acc1.address], 1);
    ...
    it("create request to zero address", async function() {
        ...
        await creationFailed(acc1, ZERO_ADDRESS, TEST_PERIODS, "HEGVesting: Zero address.");
    });

    // ----------------------------------------------------------------------------
    it("create request with zero amount/duration", async function() {
        await creationFailed(acc1, acc2.address, [[0,100],[1800,100]], "HEGVesting: Empty duration.");
        await creationFailed(acc1, acc2.address, [[1800,100],[1800,0]], "HEGVesting: Empty amount.");
    });
    ...
    it("release something one second before the first period", async function() {
        ...
        await releaseFailed(acc2, scheduleId, 1, "HEGVesting: Not enough sufficient tokens.");
    ...
    it("release a revoked vesting", async function() {
        ...
        await releaseFailed(acc2, scheduleId, 1, "HEGVesting: Schedule is revoked.");
    });
    ...
    it("release access (only admin and beneficiary car release)", async function() {
        ...
        await releaseFailed(acc3, scheduleId, 1, "HEGVesting: Only beneficiary or admin can release vested tokens.");
    });
    ...
    describe('Creation/Release tests for the first period', function () {
        ...
        it("release half of the tokens, then the rest + one extra token", async function() {
            ...
            await releaseFailed(acc2, scheduleId, amount, "HEGVesting: Not enough sufficient tokens.");
            expect(await token.balanceOf(acc2.address))
                .to.be.eq(balance);
        });
        ...
        it("release zero tokens", async function() {
            ...
            vesting.setCurrentTime(RUN_TIME + TEST_PERIODS[0][0]);
            await releaseFailed(acc2, scheduleId, 0, "HEGVesting: Zero amount.");
        });
    });
    ...
    describe('Creation/Release tests for the last period', function () {
        ...
        it("release all tokens + one extra", async function() {
            ...
            vesting.setCurrentTime(duration);
            await releaseFailed(acc2, scheduleId, amount + 1, "HEGVesting: Not enough sufficient tokens.");
            expect(await token.balanceOf(acc2.address))
                .to.be.eq(balance);
        });
    });
