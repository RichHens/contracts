...
describe('HEG Vesting: Creation vesting access tests', function () {
    ...
    it("try to release non-created schedule", async function() {
        ...
        await expect(
            ...
            .to.be.revertedWith("HEGVesting: Schedule is not created.");
    });
    ...
    describe('Request tests', function () {
        ...
        it("not enough approvals (approvals == MIN_REQUESTS_REQUIRED - 1)", async function() {
            ...
            await createFailed(admin1, testScheduleId, "HEGVesting: Not enough approves.");
        });
        ...
        it("try to create a request twice", async function() {
            ...
            await createSuccess(admin1, testScheduleId);
            await createFailed(admin1, testScheduleId, "HEGVesting: Schedule is already created.");
        });
    });
    ...
    describe('Approvals tests', function () {
        ...
        it("try to approve twice one request", async function() {
            await requestCreationSuccess(admin1, testScheduleId);
            await approveCreationRequestFailed(admin1, testScheduleId, "HEGVesting: Request is already approved.");
            await approveCreationRequestSuccess(admin2, testScheduleId);
            await approveCreationRequestFailed(admin2, testScheduleId, "HEGVesting: Request is already approved.");
        });
        ...
        it("try to approve a non-existent request", async function() {
            ...
            await requestCreationSuccess(admin1, testScheduleId);
            await approveCreationRequestFailed(admin1, wrongScheduleId, "HEGVesting: Schedule does not exist.");
        });
    ...
    describe('Revocation tests', function () {
        ...
        it("try to revoke a non-existing request", async function() {
            let wrongScheduleId = await vesting.generateScheduleId(notAdmin.address, 1);
            await requestCreationSuccess(admin1, testScheduleId);
            await revokeCreationRequestFailed(admin1, wrongScheduleId, "HEGVesting: Schedule does not exist.");
        });
        ...
        it("try to revoke already created request", async function() {
            ...
            await createSuccess(admin1, testScheduleId);
            await revokeCreationRequestFailed(admin1, testScheduleId, "HEGVesting: Schedule is already created.");
        });
    });
    ...
    describe('onlyAdmin tests', function () {
        ...
        it("create", async function() {
            await createFailed(notAdmin, testScheduleId, "HEGVesting: You are not an admin.");
        });

        // ----------------------------------------------------------------------------
        it("requestCreation", async function() {
            await requestCreationFailed(notAdmin, "HEGVesting: You are not an admin.");
        });

        // ----------------------------------------------------------------------------
        it("approveCreationRequest", async function() {
            await approveCreationRequestFailed(notAdmin, testScheduleId, "HEGVesting: You are not an admin.");
        });

        // ----------------------------------------------------------------------------
        it("revokeCreationRequest", async function() {
            await revokeCreationRequestFailed(notAdmin, testScheduleId, "HEGVesting: You are not an admin.");
        });
    });
