 ...
 * ------------------------------------------------------------------------------
 */
describe('HEG Token: Minter access tests', function () {
    ...
    describe('Request tests', function () {
        ...
        it("not enough requests (requests == MIN_REQUESTS_REQUIRED - 1)", async function() {
            ...
            await banMinterFailed(minter1, banAccount, "HEGToken: Not enough requests.");
            expect(await token.isMinter(banAccount.address))
                .to.be.eq(true);
        });
        ...
        it("two minter request for the same minter", async function() {
            ...
            await requestMinterBanFailed(minter1, banAccount, "HEGToken: The request already exists.");
        });
        ...
        it("two bans for the same minter", async function() {
            ...
            expect(await token.isMinter(banAccount.address))
                .to.be.eq(false);
            await banMinterFailed(minter1, banAccount, "HEGToken: The account is not a minter.");
        });

        // ----------------------------------------------------------------------------
        it("ban not a minter", async function() {
            await requestMinterBanFailed(minter1, notMinter, "HEGToken: The account is not a minter.");
        });

        // ----------------------------------------------------------------------------
        it("ban yourself", async function() {
            await requestMinterBanFailed(minter1, minter1, "HEGToken: It is forbidden to ban yourself.");
        });
    });
    ...
    describe('onlyMinter tests', function () {
        ...
        it("requestMinterBan", async function() {
            await requestMinterBanFailed(notMinter, minter1, "HEGToken: You are not a minter.");
        });

        // ----------------------------------------------------------------------------
        it("revokeMinterBanRequest", async function() {
            await revokeMinterBanRequestFailed(notMinter, minter1, "HEGToken: You are not a minter.");
        });

        // ----------------------------------------------------------------------------
        it("banMinter", async function() {
            await banMinterFailed(notMinter, minter1, "HEGToken: You are not a minter.");
        });
    });
