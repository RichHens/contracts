...
describe('HEN Vesting: Transfer and balance tests', function () {
  ...
  // ----------------------------------------------------------------------------
  it(TEST_TOKEN_AMOUNT + " HEN on the contract", async function() {
  ...
  it("requestCreation -> withdraw", async function() {
    ...
    await expect(vesting.create(await vesting.generateScheduleId(acc2.address, 0)))
      .to.be.revertedWith("HENVesting: Not enough sufficient tokens.");
  });
  ...
  it("requestCreation -> create -> withdraw", async function() {
    ...
    await expect(vesting.withdraw(0))
      .to.be.revertedWith("HENVesting: Not enough funds.");
